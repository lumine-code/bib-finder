...
module.exports = {
  ...
  async cache(sourceId) {
    ...
    for (const fPath of paths) {
      try {
        ...
        this.reportDiagnostics(fPath, document.diagnostics);
  ...
  reportDiagnostics(fPath, diagnostics) {
    ...
    lumine.notifications.addWarning("Bibliography source contains parsing issues", {
      detail: `${fPath} has ${diagnostics.length} ${noun}. First: line ${line}, column ${column}: ${first.message}`,
      dismissable: true,
    });
  },

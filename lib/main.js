...
module.exports = {
  ...
  toggle(sourceId) {
    ...
    return atom.modals.toggle({
      ...
      // The old view held one query editor for the module's lifetime and only
      // ran `selectAll()` on show, so a reopened list still carried the last
      // query with it, ready to be typed over. `preserveQuery` restores that.
      preserveQuery: true,
      help: HELP,
  ...
  insert(item, mode, target) {
    ...
    // The editor that had focus, not the centre pane's: `f9` is bound on bare
    // `atom-text-editor`, so it fires in panel and dock mini editors too, and
    // a citation must land in the one the user was typing in.
    const editor = target.focusedEditor;
